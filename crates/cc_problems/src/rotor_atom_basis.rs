...
impl TRAMRotorAtomBasis {
    ...
    pub fn pes_polarization_legendre_component(&self, lambda: u32, s_tot: HalfU32) -> impl Fn(BasisElementsRef) -> Operator + use<> {
        ...
        move |b| operator_mel!(b, [s_r_id, s_a_id, n_tot_id], |[s_r, s_a, n_tot]| {
            if n_tot.bra.pair.0 == n_tot.ket.pair.0 {
                ...
                f64::sqrt(6.0) * dot_product_separation(
                    ...
                    n_tot,
                    2,
                    |q| tensor_product_separation(s_r, s_a, 1, 1, spin!(2, q), 
...
fn spin_rot(s_id: BasisId<Spin>, n_tot_id: BasisId<AngularCoupled>) -> impl Fn(BasisElementsRef) -> Operator + 'static + Send + Sync {
    move |b| operator_mel!(b, [s_id, n_tot_id], |[s, n_tot]| {
        if n_tot.bra.pair == n_tot.ket.pair {
            ...
            dot_product_separation(
                ...
                |q| wigner_eckart_factor(n_tot, spin!(1, q))
...
fn aniso_hifi(
    ...
    move |b| operator_mel!(b, [s_id, i_id, n_tot_id], |[s, i, n_tot]| {
        if n_tot.bra.pair.1 == n_tot.ket.pair.1 {
            ...
            f64::sqrt(6.0) / 3.0 * dot_product_separation(
                si, 
                n_tot,
                2,
                |q| tensor_product_separation(s, i, 1, 1, spin!(2, q), 
